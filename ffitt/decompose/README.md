...
1. Find every peak and every valley of the signal, with `ffitt/util`.
2. Draw a spline through the peaks and a spline through the valleys, with
   `ffitt/interpolate`. Those two curves are the envelopes.
...
through the time, give them to `hht` in `ffitt/transform`. Together they are the
