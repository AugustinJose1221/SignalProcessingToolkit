...
def detrended(lib, block, kind):
    ...
    assert lib.detrend_block(ffitt.float_array(block), output, size, kind)
    return list(output)
...
def test_a_block_with_the_level_out_has_no_level_left(lib, block):
    ...
    answer = detrended(lib, block, ffitt.DETREND_CONSTANT)
    total = sum(answer)
...
def test_a_block_with_the_drift_out_has_no_level_and_no_drift_left(lib, block):
    ...
    """
    answer = detrended(lib, block, ffitt.DETREND_LINEAR)
    size = len(answer)
...
@given(BLOCKS, st.sampled_from([ffitt.DETREND_CONSTANT, ffitt.DETREND_LINEAR]))
...
def test_the_trend_that_is_reported_is_the_trend_that_is_taken(lib, block):
    ...
    size = len(block)
    offset = ffitt.real_buffer(1)
    slope = ffitt.real_buffer(1)

    assert lib.detrend_trend(ffitt.float_array(block), size,
                             ffitt.DETREND_LINEAR, offset, slope)
...
def test_adding_a_trend_to_a_block_does_not_change_what_is_left(lib, block,
    ...
    plain = detrended(lib, block, ffitt.DETREND_LINEAR)
    after = detrended(lib, tilted, ffitt.DETREND_LINEAR)
...
def test_removing_a_known_trend_and_putting_it_back_gives_the_block(lib, block):
    ...
    taken = ffitt.real_buffer(size)

    assert lib.detrend_remove(ffitt.float_array(block), taken, size, offset,
                              slope)
...
def test_a_block_of_one_has_a_level_and_no_direction(lib, block):
    ...
    one = ffitt.float_array([block[0]])
    output = ffitt.real_buffer(1)

    assert lib.detrend_block(one, output, 1, ffitt.DETREND_CONSTANT)
    assert not lib.detrend_block(one, output, 1, ffitt.DETREND_LINEAR)
...
def test_only_the_two_kinds_that_exist_are_taken(lib, kind):
    ...
    expected = kind in (ffitt.DETREND_CONSTANT, ffitt.DETREND_LINEAR)
